...
function M.setup(opts)
    ...
    vim.api.nvim_create_user_command('PullFtp', function()
        ...
        if projectRoot then
            ...
            if settings then
                vim.cmd("!lftp -e 'mirror --no-empty-dirs -n ".. M.includeString .." --parallel=".. M.concurrent_transfers .." "..settings[5].. " ".. projectRoot .."; exit;' '"..settings[1].."://"..settings[3]..":"..settings[4].."@"..settings[2].."'")
            end
        end
    end, {})
end
...
function M.uploadCallback(ev)
    ...
    if projectRoot then
        ...
        if settings then
            ...
            vim.cmd('silent !lftp -e "put '..ev.match..' -o ./'..settings[5]..relativePath..'; exit;" "'..settings[1]..'://'..settings[3]..':'..settings[4]..'@'..settings[2]..'"')
        end
    end
end
...
function M.getProjectSettings(path)
    ...
    vim.api.nvim_err_writeln("ftp-redux: .conn file should contain exactly five lines:\nprotocol, host, user, pass, root dir")
    return false
end

function M.getProjectRoot(path)
    if vim.fn.filereadable(path .. '/.conn') ~= 0 then return path end --check if dir has .conn file
    while true do
